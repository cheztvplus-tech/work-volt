...
window.WorkVoltPages['settings'] = function(container) {
  ...
  window.settingsSave = function() {
    ...
    setTimeout(function() { window.settingsTestConnection(); }, 500);
  };
  ...
  window.settingsTestConnection = async function() {
    ...
    if (!url || !sheetId) {
      render({ ok: false, message: 'Please enter GAS URL and Sheet ID' });
      return;
    }
    ...
    try {
      ...
      var pingRes = await fetch(pingUrl.toString(), { cache: 'no-cache', mode: 'cors' });
      
      if (!pingRes.ok) throw new Error('GAS server error: ' + pingRes.status);
      ...
      var checkRes = await fetch(checkUrl.toString(), { cache: 'no-cache', mode: 'cors' });
      if (!checkRes.ok) throw new Error('Sheet check failed: ' + checkRes.status);
      
      var checkData = await checkRes.json();
      if (checkData.error) throw new Error(checkData.error);

      render({ ok: true, message: '✓ Connected successfully to your Google Sheet!' });
      
      if (btn) {
        setTimeout(function() {
          if (btn) { 
            btn.disabled = false; 
            btn.innerHTML = '<i class="fas fa-vial text-sm"></i> Test Connection'; 
          }
        }, 1000);
      }
    } catch(e) {
      render({ ok: false, message: 'Connection failed: ' + e.message + '. Check URL, Sheet ID, and API Secret.' });
