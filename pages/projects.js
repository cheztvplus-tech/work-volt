...
window.WorkVoltPages['projects'] = function(container) {
  ...
  var myId   = (function() { try { return window.WorkVolt.user().id || window.WorkVolt.user().user_id || ''; } catch(e) { return ''; } })();
  ...
  var taskFilter  = { status: '', priority: '', assigned_to: '', quick: '' };
  var taskSearch  = '';
  var focusMode   = false;
  ...
  // ── Supabase API layer ───────────────────────────────────────────
  // Every former GAS api() call is replaced with direct WorkVoltDB calls.
  // The functions below mirror the old GAS path semantics so the rest of
  // the module (detail loader, quick-create, etc.) changes minimally.
  ...
  async function apiProjectsList() {
    ...
    if (tasksInstalled()) {
      ...
      rows = rows.map(function(p) {
        ...
        var overdue = pt.filter(function(t){return t.due_date&&t.due_date<now&&t.status!=='Done'&&t.status!=='Cancelled';}).length;
        p.task_count   = pt.length;
        p.tasks_done   = done;
        p.overdue_count= overdue;
        if (pt.length) p.progress = Math.round((done/pt.length)*100);
        return p;
      });
    }
    return rows;
  ...
  async function apiProjectCreate(params) {
    var row = {
      ...
      owner_id:    params.owner_id    || myId || null,
  ...
  // members
  async function apiMembersList(pid) {
    return db.list('project_members', { project_id: pid });
  }

  async function apiMemberAdd(pid, uid, role) {
  ...
  // activity
  async function apiActivityList(pid, limit) {
    return db.list('project_activity', { project_id: pid },
      { order: 'created_at', limit: limit || 30 });
  }

  async function apiActivityLog(pid, action, detail) {
  ...
  // stats — computed client-side from tasks
  function computeStats(tasks) {
    ...
    return {
      total, done, in_progress: inProgress, overdue, progress,
  ...
  async function apiTaskCreate(params) {
    var row = {
      title:           params.title,
      description:     params.description     || null,
      status:          params.status          || 'To Do',
      priority:        params.priority        || 'Medium',
      assignee:        params.assigned_to     || null,  // tasks.js uses 'assignee'
      assigned_to:     params.assigned_to     || null,
      due_date:        params.due_date        || null,
      estimated_hours: params.estimated_hours ? parseFloat(params.estimated_hours) : null,
      linked_task_id:  params.linked_task_id  || null,
      project:         params.project_id      || null,  // tasks.js uses 'project'
    ...
    var html =
      ...
      '<div class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between gap-3 flex-wrap">'+
        ...
        '<div class="flex items-center gap-2">'+
          (isAdmin()?'<button id="btn-new-project" class="btn-primary text-sm"><i class="fas fa-plus text-xs mr-1"></i>New Project</button>':'')+
        '</div>'+
      '</div>'+
      ...
      '<div id="proj-grid" class="p-6">'+
        '<div class="flex items-center justify-center py-20 text-slate-300"><i class="fas fa-circle-notch fa-spin text-3xl"></i></div>'+
      '</div>'+
      '</div>';
  ...
  function renderGrid(statusFilter, searchQ) {
    ...
    if(!rows.length) {
      grid.innerHTML='<div class="flex flex-col items-center justify-center py-24 text-slate-300">'+
    ...
    grid.innerHTML='<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:1.25rem">'+
  ...
  function renderProjectCard(p) {
    ...
    var memberAvatars='';
    if(p.owner_id) memberAvatars='<div class="flex -space-x-1.5">'+userAvatar(p.owner_id,'w-6 h-6 text-[10px] ring-2 ring-white')+'</div>';
      ...
      '<div class="p-5">'+
        ...
        '<div class="flex items-center gap-2 mb-4 flex-wrap">'+
          ...
          (p.priority
            ?'<span class="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full '+
              ...
              (PRIORITY_COLORS[p.priority]||PRIORITY_COLORS['Medium']).text+'">'+esc(p.priority)+'</span>'
            :'')+
        '</div>'+
        '<div class="mb-4">'+
        ...
        '<div class="flex items-center justify-between pt-3 border-t border-slate-100">'+
          '<div class="flex items-center gap-3">'+
            (p.due_date
              ?'<span class="flex items-center gap-1 text-[11px] text-slate-500"><i class="fas fa-calendar-alt text-[10px] text-slate-300"></i>'+fmtDate(p.due_date)+'</span>'
              :'<span class="text-[11px] text-slate-300">No deadline</span>')+
            (tasksInstalled()&&p.task_count
              ?'<span class="flex items-center gap-1 text-[11px] text-slate-500"><i class="fas fa-check-square text-[10px] text-slate-300"></i>'+p.task_count+'</span>'
              :'')+
          '</div>'+
          memberAvatars+
        '</div>'+
  ...
  async function openProjectDetail(pid) {
    ...
    activityCache=[];
    statsCache={};
    taskFilter={status:'',priority:'',assigned_to:'',quick:''};
    taskSearch='';
    focusMode=false;
    ...
    try {
      ...
      activityCache = activityRows;

      if(tasksInstalled()) {
        var taskRows = await apiTasksList(pid).catch(function(){return [];});
  ...
  function renderDetailShell(pid) {
    container.innerHTML=
      '<div class="min-h-full bg-slate-50">'+
        ...
        '</div>'+
        '<div class="flex items-center justify-center py-32 text-slate-300"><i class="fas fa-circle-notch fa-spin text-4xl"></i></div>'+
      '</div>';
  ...
  function renderDetailFull() {
    ...
    var sc   =STATUS_CONFIG[p.status]||STATUS_CONFIG['Planning'];
    var h    =healthIndicator(p);
    var pct  =parseInt(statsCache.progress||p.progress)||0;
    ...
    if(membersCache.length>5) teamAvatars+='<span class="w-8 h-8 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center text-[10px] font-bold ring-2 ring-white">+'+(membersCache.length-5)+'</span>';
    ...
    container.innerHTML=
        ...
        '<div class="px-6 py-5">'+
          '<div class="flex items-start justify-between gap-4 mb-4">'+
            ...
            '<div class="flex items-center gap-2 flex-shrink-0">'+
              ...
              (isAdmin()?'<button id="btn-edit-project" class="flex items-center gap-1.5 text-xs font-bold px-3 py-2 rounded-xl border border-slate-200 bg-white text-slate-600 hover:border-blue-400 hover:text-blue-600 transition-all"><i class="fas fa-pen text-[11px]"></i>Edit</button>':'')+
              (tasksInstalled()?'<button id="btn-new-task" class="btn-primary text-xs"><i class="fas fa-plus text-[11px] mr-1"></i>Add Task</button>':'')+
            '</div>'+
          '</div>'+
          ...
          '<div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(140px,1fr))">'+
            ...
            '<div class="flex items-center gap-2.5">'+
              '<div class="w-10 h-10 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center flex-shrink-0"><i class="fas fa-calendar-alt text-slate-400 text-sm"></i></div>'+
              '<div>'+
      ...
      '<div class="flex flex-1 overflow-hidden" style="min-height:0">'+
        '<div class="w-56 bg-white border-r border-slate-200 flex-shrink-0 overflow-y-auto" id="proj-left-panel">'+renderLeftPanel()+'</div>'+
        '<div class="flex-1 flex flex-col overflow-hidden" id="proj-center">'+renderCenterPanel()+'</div>'+
        '<div class="w-64 bg-white border-l border-slate-200 flex-shrink-0 overflow-y-auto" id="proj-right-panel">'+renderRightPanel()+'</div>'+
      '</div>'+
      '</div>';

    container.querySelector('#btn-back').addEventListener('click', function(){view='list';activeProject=null;renderListView();});
    container.querySelector('#btn-focus-mode').addEventListener('click', function(){focusMode=!focusMode;renderDetailFull();});
    if(isAdmin()){
      var eb=container.querySelector('#btn-edit-project');
      if(eb) eb.addEventListener('click', function(){openProjectForm(activeProject);});
    }
    if(tasksInstalled()){
      var nt=container.querySelector('#btn-new-task');
      if(nt) nt.addEventListener('click', function(){openNewTaskForm();});
    }
    wireCenterPanel();
  }
  ...
  function renderLeftPanel() {
    ...
    if(tasksInstalled()){
      ...
      var quickLinks=[
        ...
        {k:'mine',   icon:'fa-user',         lbl:'Assigned to Me',count:Object.values(tasksCache).filter(function(t){return (t.assigned_to||t.assignee)===myId;}).length},
      ...
      if(Object.keys(breakdown).length){
        ...
        TASK_STATUSES.forEach(function(s){
          var cnt=breakdown[s]||0; if(!cnt) return;
          var tc=TASK_STATUS_COLORS[s]||TASK_STATUS_COLORS['To Do'];
          html+='<div class="flex items-center justify-between mb-1.5">'+
            '<span class="flex items-center gap-1.5 text-[11px] text-slate-600"><span class="w-1.5 h-1.5 rounded-full flex-shrink-0" style="background:'+tc.dot+'"></span>'+s+'</span>'+
    ...
    if(membersCache.length){
      ...
      if(isAdmin()) html+='<button id="btn-add-member" class="w-full mt-1 text-[11px] font-semibold text-blue-600 hover:text-blue-800 flex items-center gap-1 px-2 py-1.5 rounded-lg hover:bg-blue-50 transition-colors"><i class="fas fa-user-plus text-[10px]"></i>Add member</button>';
    }
    html+='</div>';
    return html;
  }
  ...
  function renderCenterPanel() {
    ...
    return '<div class="bg-white border-b border-slate-200 px-4 py-2.5 flex items-center gap-2 flex-wrap flex-shrink-0">'+
        '<div class="flex bg-slate-100 rounded-lg p-0.5">'+viewBtns+'</div>'+
        (focusMode?'<span class="flex items-center gap-1.5 text-xs font-bold px-2.5 py-1.5 rounded-lg bg-indigo-50 text-indigo-700 border border-indigo-200"><i class="fas fa-crosshairs text-[10px]"></i>Focus Mode — showing only your tasks today</span>':'')+
        '<div class="ml-auto flex items-center gap-2">'+
          '<select id="tf-status" class="field text-xs py-1.5" style="width:7.5rem">'+
            '<option value="">All Statuses</option>'+
            TASK_STATUSES.map(function(s){return '<option value="'+s+'"'+(taskFilter.status===s?' selected':'')+'>'+s+'</option>';}).join('')+
          '</select>'+
          '<select id="tf-priority" class="field text-xs py-1.5" style="width:7rem">'+
            '<option value="">All Priorities</option>'+
            TASK_PRIORITIES.map(function(p){return '<option value="'+p+'"'+(taskFilter.priority===p?' selected':'')+'>'+p+'</option>';}).join('')+
          '</select>'+
          '<div class="relative">'+
      ...
      (tasksInstalled()&&isAdmin()
        ?'<div class="bg-white border-b border-slate-100 px-4 py-2 flex items-center gap-2 flex-shrink-0">'+
          ...
          '</div>'
        :'')+
      '<div id="task-content" class="flex-1 overflow-y-auto p-4">'+renderTaskContent(tasks)+'</div>';
  }
  ...
  function getFilteredTasks() {
    ...
    if(taskFilter.assigned_to) tasks=tasks.filter(function(t){return (t.assigned_to||t.assignee)===taskFilter.assigned_to;});
    if(taskFilter.quick==='mine')    tasks=tasks.filter(function(t){return (t.assigned_to||t.assignee)===myId;});
  ...
  function renderTaskContent(tasks) {
    if(!tasksInstalled()) return '<div class="flex flex-col items-center justify-center py-20 text-slate-300"><i class="fas fa-check-circle text-5xl mb-4 opacity-30"></i><p class="font-semibold text-slate-500">Tasks module not installed</p><p class="text-sm mt-1">Install the Tasks module to manage project tasks.</p></div>';
  ...
  function renderTaskListView(tasks) {
    if(!tasks.length) return '<div class="flex flex-col items-center justify-center py-16 text-slate-300"><i class="fas fa-check-circle text-4xl mb-3 opacity-30"></i><p class="font-semibold text-slate-500">'+(focusMode?'No tasks assigned to you today':'No tasks yet')+'</p>'+(isAdmin()&&!focusMode?'<p class="text-sm mt-1 text-slate-400">Use the quick add bar above</p>':'')+'</div>';
    var groups={}; TASK_STATUSES.forEach(function(s){groups[s]=[];});
    tasks.forEach(function(t){ if(!groups[t.status]) groups[t.status]=[]; groups[t.status].push(t); });
    return TASK_STATUSES.filter(function(s){return groups[s]&&groups[s].length;}).map(function(s){
      ...
      return '<div class="mb-4">'+
        ...
        '</div>'+
        '<div class="flex flex-col gap-1.5 '+(isDone?'opacity-60':'')+'">'+ groups[s].map(function(t){return renderTaskRow(t);}).join('') +'</div>'+
      '</div>';
    }).join('');
  }
  ...
  function renderTaskRow(t) {
    ...
    return '<div data-task-id="'+esc(t.id)+'" class="group bg-white rounded-xl border '+(over?'border-red-200 bg-red-50/30':'border-slate-200')+' px-3.5 py-2.5 flex items-center gap-3 hover:shadow-sm transition-all cursor-pointer">'+
      ...
      '</div>'+
      '<div class="flex items-center gap-2 flex-shrink-0">'+
        (aid?userAvatar(aid,'w-6 h-6 text-[10px]'):'')+
        '<div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">'+
          '<button data-task-action="edit" data-task-id="'+esc(t.id)+'" class="w-6 h-6 rounded-lg bg-slate-100 hover:bg-blue-100 hover:text-blue-600 text-slate-400 flex items-center justify-center text-[10px] border-none cursor-pointer transition-colors"><i class="fas fa-pen"></i></button>'+
          (t.status!=='Done'?'<button data-task-action="done" data-task-id="'+esc(t.id)+'" class="w-6 h-6 rounded-lg bg-slate-100 hover:bg-green-100 hover:text-green-600 text-slate-400 flex items-center justify-center text-[10px] border-none cursor-pointer transition-colors"><i class="fas fa-check"></i></button>':'')+
        '</div>'+
      '</div>'+
    '</div>';
  }
      ...
      TASK_STATUSES.map(function(s){
        ...
        return '<div class="flex-shrink-0 w-64 rounded-2xl border '+kc.border+' overflow-hidden flex flex-col">'+
          '<div class="'+kc.head+' px-3 py-2.5 flex items-center justify-between border-b '+kc.border+'">'+
            '<div class="flex items-center gap-2"><span class="w-2 h-2 rounded-full '+kc.dot+'"></span><span class="text-xs font-extrabold text-slate-700">'+s+'</span><span class="text-[10px] font-bold bg-white/60 px-1.5 py-px rounded-full text-slate-600">'+cols.length+'</span></div>'+
          '</div>'+
          '<div class="p-2 flex flex-col gap-2 bg-white/50 flex-1 overflow-y-auto" style="max-height:500px">'+
            (cols.length?cols.map(function(t){
              var over=isOverdue(t);
              var pc=PRIORITY_COLORS[t.priority]||PRIORITY_COLORS['Medium'];
              var aid=t.assigned_to||t.assignee;
              return '<div data-task-id="'+esc(t.id)+'" class="bg-white rounded-xl border '+(over?'border-red-200':'border-slate-200')+' p-3 cursor-pointer hover:shadow-sm transition-all">'+
                ...
                '<div class="flex items-center justify-between">'+
                  ...
                  (aid?userAvatar(aid,'w-5 h-5 text-[9px]'):'')+
                '</div>'+
                (t.due_date?'<p class="text-[10px] '+(over?'text-red-500 font-bold mt-1':'text-slate-400 mt-1')+'">'+fmtDate(t.due_date)+'</p>':'')+
              '</div>';
            }).join(''):'<div class="text-center py-6 text-slate-300 text-xs">Drop tasks here</div>')+
          '</div>'+
        '</div>';
      }).join('')+
    '</div>';
  ...
  function renderCalendarView(tasks) {
    ...
    tasks.forEach(function(t){
      ...
      if(d.getFullYear()===year&&d.getMonth()===month){var k=d.getDate(); if(!byDay[k]) byDay[k]=[]; byDay[k].push(t);}
    });
    ...
    for(var d=1;d<=daysInMonth;d++){
      ...
      cells+='<div class="bg-white rounded-lg border border-slate-100 p-1.5 min-h-[60px] hover:border-slate-300 transition-colors">'+
        ...
        dt.slice(0,2).map(function(t){var pc=PRIORITY_COLORS[t.priority]||PRIORITY_COLORS['Medium']; return '<div data-task-id="'+esc(t.id)+'" class="text-[9px] font-semibold px-1 py-0.5 rounded mb-0.5 cursor-pointer truncate '+pc.bg+' '+pc.text+'">'+esc(t.title)+'</div>';}).join('')+
        (dt.length>2?'<p class="text-[9px] text-slate-400">+'+(dt.length-2)+' more</p>':'')+
      '</div>';
    }
    return '<div>'+
      '<div class="flex items-center justify-between mb-3"><h3 class="text-sm font-extrabold text-slate-800">'+monthName+'</h3></div>'+
      '<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:4px" class="mb-1">'+days.map(function(d){return '<div class="text-[10px] font-extrabold text-slate-400 text-center py-1">'+d+'</div>';}).join('')+'</div>'+
  ...
  function renderRightPanel() {
    var breakdown={}, byPriority={}, byAssignee={};
    try{breakdown=JSON.parse(statsCache.by_status||'{}');}catch(e){}
    ...
    if(tasksInstalled()){
      html+='<div><p class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-3">Overview</p>';
      [{label:'Total Tasks',val:statsCache.total||0,icon:'fa-tasks',color:'text-blue-500'},
       ...
       {label:'Overdue',    val:statsCache.overdue||0,icon:'fa-fire',color:'text-red-500'},
        ...
        html+='<div class="flex items-center justify-between py-1.5">'+
          '<span class="flex items-center gap-2 text-xs text-slate-600"><i class="fas '+sc.icon+' '+sc.color+' text-[10px] w-3"></i>'+sc.label+'</span>'+
          '<span class="text-xs font-extrabold text-slate-800">'+sc.val+'</span></div>';
      });
      html+='</div>';
      if(Object.keys(byPriority).length){
        html+='<div><p class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-3">By Priority</p>';
        var totalT=Object.values(tasksCache).length||1;
        TASK_PRIORITIES.forEach(function(p){
          var cnt=byPriority[p]||0; if(!cnt) return;
          var pc=PRIORITY_COLORS[p]||PRIORITY_COLORS['Medium'];
          var pct=Math.round((cnt/totalT)*100);
          html+='<div class="mb-2.5"><div class="flex items-center justify-between mb-1"><span class="text-[11px] font-semibold text-slate-600">'+p+'</span><span class="text-[11px] font-bold text-slate-700">'+cnt+'</span></div>'+
            '<div style="background:#f1f5f9;border-radius:9999px;height:5px"><div style="width:'+pct+'%;height:5px;border-radius:9999px;background:'+pc.dot+';transition:width .5s"></div></div></div>';
        });
        html+='</div>';
      }
      if(Object.keys(byAssignee).length){
        html+='<div><p class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-3">Workload</p>';
        var maxLoad=Math.max.apply(null,Object.values(byAssignee))||1;
        Object.keys(byAssignee).slice(0,5).forEach(function(uid){
          var cnt=byAssignee[uid], pct=Math.round((cnt/maxLoad)*100);
          html+='<div class="flex items-center gap-2 mb-2">'+userAvatar(uid,'w-6 h-6 text-[9px]')+
            '<div class="flex-1 min-w-0"><div class="flex items-center justify-between mb-0.5"><p class="text-[11px] font-semibold text-slate-600 truncate">'+esc(userName(uid))+'</p><span class="text-[10px] font-bold text-slate-500">'+cnt+'</span></div>'+
            '<div style="background:#f1f5f9;border-radius:9999px;height:4px"><div style="width:'+pct+'%;height:4px;border-radius:9999px;background:'+(cnt>maxLoad*0.8?'#f97316':'#3b82f6')+'"></div></div></div></div>';
        });
        html+='</div>';
      }
    }
    if(activityCache.length){
      html+='<div><p class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-3">Activity</p>';
      activityCache.slice(0,8).forEach(function(a){
        html+='<div class="flex gap-2 mb-3">'+userAvatar(a.user_id,'w-6 h-6 text-[9px] flex-shrink-0')+
          '<div class="flex-1 min-w-0"><p class="text-[11px] text-slate-600 leading-snug"><span class="font-semibold text-slate-800">'+esc(userName(a.user_id))+'</span> '+esc(a.action)+(a.detail?' — <span class="text-slate-500">'+esc(String(a.detail||'').substring(0,60))+'</span>':'')+'</p>'+
          '<p class="text-[10px] text-slate-400 mt-0.5">'+timeAgo(a.created_at)+'</p></div></div>';
      });
      html+='</div>';
    }
    html+='</div>';
  ...
  function wireCenterPanel() {
    ...
    if(srch) srch.addEventListener('input', function(){taskSearch=this.value; clearTimeout(_searchTimer); _searchTimer=setTimeout(refreshCenter,200);});
    var qa=container.querySelector('#proj-quick-add');
    if(qa) qa.addEventListener('keydown', function(e){
      if(e.key==='Enter'&&this.value.trim()){var t=this.value.trim(); this.value=''; quickCreateTask(t);}
    });
    ...
    tc.addEventListener('click', function(e){
      var ab=e.target.closest('[data-task-action]');
      if(ab){
        e.stopPropagation();
        var id=ab.dataset.taskId, act=ab.dataset.taskAction;
  ...
  // ── Quick create / update task ───────────────────────────────────
  async function quickCreateTask(title) {
    try {
      var created=await apiTaskCreate({
        title:      title,
      ...
      refreshCenter();
      refreshRightPanel();
      apiActivityLog(activeProject.id,'created task',title);
  ...
  async function quickUpdateTask(id, status) {
    try {
      ...
      refreshCenter();
      refreshRightPanel();
      apiActivityLog(activeProject.id,'marked '+status.toLowerCase(),tasksCache[id]?tasksCache[id].title:id);
    } catch(e){ toast(e.message,'error'); }
  }
  ...
  function openProjectForm(proj) {
    ...
    var statusOpts  =STATUSES.map(function(s){return '<option value="'+s+'"'+(s===(v('status')||'Planning')?' selected':'')+'>'+s+'</option>';}).join('');
    var priorityOpts=PRIORITIES.map(function(p){return '<option value="'+p+'"'+(p===(v('priority')||'Medium')?' selected':'')+'>'+p+'</option>';}).join('');
    ...
    var ownerOpts='<option value="">— Select owner —</option>'+
      usersCache.map(function(u){var uid=u.user_id||u.id; return '<option value="'+esc(uid)+'"'+(uid===v('owner_id')?' selected':'')+'>'+esc(u.name||u.email)+'</option>';}).join('');
    var html=
      '<div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">'+
        '<h3 class="font-extrabold text-slate-900 flex items-center gap-2"><i class="fas fa-folder-open text-blue-500"></i>'+(isEdit?'Edit Project':'New Project')+'</h3>'+
      ...
      '<div class="px-6 py-5 flex flex-col gap-4">'+
        ...
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem">'+
          '<div><label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Status</label><select id="pf-status" class="field text-sm">'+statusOpts+'</select></div>'+
          '<div><label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Priority</label><select id="pf-priority" class="field text-sm">'+priorityOpts+'</select></div>'+
        '</div>'+
        '<div><label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Owner</label><select id="pf-owner" class="field text-sm">'+ownerOpts+'</select></div>'+
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem">'+
          '<div><label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Start Date</label><input id="pf-start" class="field text-sm" type="date" value="'+esc(v('start_date'))+'"></div>'+
          '<div><label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Due Date</label><input id="pf-due" class="field text-sm" type="date" value="'+esc(v('due_date'))+'"></div>'+
        '</div>'+
    ...
    document.querySelectorAll('.color-swatch').forEach(function(sw){
      sw.addEventListener('click', function(){
        document.querySelectorAll('.color-swatch').forEach(function(s){s.classList.remove('border-slate-800','scale-110');s.classList.add('border-transparent');});
        this.classList.add('border-slate-800','scale-110'); this.classList.remove('border-transparent');
    ...
    document.getElementById('pf-save').addEventListener('click', async function(){
      ...
      var btn=this; btn.disabled=true; btn.innerHTML='<i class="fas fa-circle-notch fa-spin text-xs mr-1"></i>Saving…';
      modalStatus('',null);
      var params={
        name:        name,
        ...
        owner_id:    document.getElementById('pf-owner').value||myId||null,
        start_date:  document.getElementById('pf-start').value||null,
        due_date:    document.getElementById('pf-due').value||null,
        budget:      document.getElementById('pf-budget').value||null,
        tags:        document.getElementById('pf-tags').value||null,
        color:       selectedColor,
      };
      try {
        if(isEdit){
          var updated=await apiProjectUpdate(proj.id, params);
          activeProject=Object.assign({},activeProject,params);
          var idx=projectsCache.findIndex(function(p){return String(p.id)===String(proj.id);});
          if(idx>=0) projectsCache[idx]=Object.assign({},projectsCache[idx],params);
          modalStatus('Project updated ✓',true);
          setTimeout(function(){closeModal(); openProjectDetail(proj.id);},600);
        } else {
          var created=await apiProjectCreate(params);
          modalStatus('Project created ✓',true);
          setTimeout(function(){closeModal(); openProjectDetail(created.id);},600);
        }
      } catch(e){
  ...
  function confirmDeleteProject(pid, name) {
    showModal(
      '<div class="px-6 py-5 border-b border-slate-100"><h3 class="font-extrabold text-slate-900">Delete Project</h3></div>'+
      '<div class="px-6 py-5">'+
        '<div id="pm-status"></div>'+
    ...
    document.getElementById('del-confirm').addEventListener('click', async function(){
      ...
      try {
        await apiProjectDelete(pid);
        projectsCache=projectsCache.filter(function(p){return String(p.id)!==String(pid);});
        closeModal(); view='list'; activeProject=null; renderListView();
        toast('Project deleted','info');
      } catch(e){ modalStatus(e.message,false); btn.disabled=false; btn.innerHTML='Delete Project'; }
    });
  }

  // ── Task form ────────────────────────────────────────────────────
  function openNewTaskForm() { openEditTaskForm(null); }

  function openEditTaskForm(task) {
    var isEdit=!!task;
    function v(k){return task?(task[k]||''):'';}
    var statusOpts  =TASK_STATUSES.map(function(s){return '<option value="'+s+'"'+(s===(v('status')||'To Do')?' selected':'')+'>'+s+'</option>';}).join('');
    var priorityOpts=TASK_PRIORITIES.map(function(p){return '<option value="'+p+'"'+(p===(v('priority')||'Medium')?' selected':'')+'>'+p+'</option>';}).join('');
    var assigneeOpts='<option value="">— Unassigned —</option>'+
      usersCache.map(function(u){var uid=u.user_id||u.id; return '<option value="'+esc(uid)+'"'+(uid===(v('assigned_to')||v('assignee'))?' selected':'')+'>'+esc(u.name||u.email)+'</option>';}).join('');
    ...
    if(tasksInstalled()){
      ...
      linkedTaskField='<div><label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Linked Task</label>'+
        '<div class="flex gap-2"><select id="tf-linked-task" class="field text-sm flex-1">'+taskOpts+'</select>'+
        '<button type="button" id="tf-clear-linked" class="w-9 h-9 flex-shrink-0 rounded-xl flex items-center justify-center text-sm cursor-pointer" style="border:1px solid #e2e8f0;background:#fff;color:#94a3b8"><i class="fas fa-times"></i></button></div></div>';
    }
    var html=
      '<div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">'+
        ...
        (isEdit&&tasksInstalled()
          ?'<button id="tf-open-in-tasks" class="flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1.5 rounded-lg border border-blue-200 bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors cursor-pointer"><i class="fas fa-external-link-alt text-[10px]"></i>Open in Tasks</button>'
          :'')+
      ...
      '<div class="px-6 py-5 flex flex-col gap-4">'+
        ...
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem">'+
          '<div><label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Status</label><select id="tf-status" class="field text-sm">'+statusOpts+'</select></div>'+
          '<div><label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Priority</label><select id="tf-priority" class="field text-sm">'+priorityOpts+'</select></div>'+
        '</div>'+
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem">'+
          '<div><label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Assigned To</label>'+
          '<div class="flex gap-2"><select id="tf-assignee" class="field text-sm flex-1">'+assigneeOpts+'</select>'+
          '<button type="button" id="tf-clear-assignee" class="w-9 h-9 flex-shrink-0 rounded-xl flex items-center justify-center text-sm cursor-pointer" style="border:1px solid #e2e8f0;background:#fff;color:#94a3b8"><i class="fas fa-user-times"></i></button></div></div>'+
        ...
        linkedTaskField+
        '<div class="flex gap-3 pt-2">'+
          (isEdit&&isAdmin()?'<button id="tf-delete" class="px-4 py-2.5 rounded-xl bg-red-50 text-red-600 font-bold text-sm hover:bg-red-100 transition-colors border-none cursor-pointer"><i class="fas fa-trash text-xs mr-1"></i>Delete</button>':'')+
    ...
    var oit=document.getElementById('tf-open-in-tasks');
    if(oit) oit.addEventListener('click', function(){closeModal(); openTaskInModule(task);});
    var ca=document.getElementById('tf-clear-assignee');
    if(ca) ca.addEventListener('click', function(){document.getElementById('tf-assignee').value='';});
    var cl=document.getElementById('tf-clear-linked');
    if(cl) cl.addEventListener('click', function(){document.getElementById('tf-linked-task').value='';});
    if(isEdit&&isAdmin()){
      var delBtn=document.getElementById('tf-delete');
      if(delBtn) delBtn.addEventListener('click', async function(){
        if(!confirm('Delete task "'+esc(task.title||'')+'"?')) return;
        try {
          await apiTaskDelete(task.id);
          delete tasksCache[task.id];
          statsCache=computeStats(Object.values(tasksCache));
          closeModal(); refreshCenter(); refreshRightPanel();
    ...
    document.getElementById('tf-save').addEventListener('click', async function(){
      ...
      var btn=this; btn.disabled=true; btn.innerHTML='<i class="fas fa-circle-notch fa-spin text-xs mr-1"></i>Saving…';
      modalStatus('',null);
      var linkedEl=document.getElementById('tf-linked-task');
      var params={
        title:           title,
        ...
        estimated_hours: document.getElementById('tf-est').value||null,
        project_id:      activeProject?activeProject.id:null,
        linked_task_id:  linkedEl?linkedEl.value||null:null,
      };
      try {
        if(isEdit){
          await apiTaskUpdate(task.id, params);
          tasksCache[task.id]=Object.assign({},tasksCache[task.id]||{},params,{id:task.id});
          modalStatus('Task saved ✓',true);
        } else {
          var created=await apiTaskCreate(params);
          tasksCache[created.id]=created;
        ...
        setTimeout(function(){
          closeModal(); refreshCenter(); refreshRightPanel();
          if(activeProject) apiActivityLog(activeProject.id,isEdit?'updated task':'created task',title);
        },500);
      } catch(e){
        modalStatus(e.message,false);
        btn.disabled=false;
  ...
  // ── Open task in Tasks module ────────────────────────────────────
  function openTaskInModule(task) {
    if(!task) return;
    window._wvDeepLink={module:'tasks',id:task.id};
    if(typeof showModule==='function') { showModule('tasks'); }
    else if(window.WorkVolt&&typeof window.WorkVolt.navigate==='function') { window.WorkVolt.navigate('tasks'); }
    else {
      var navLink=document.querySelector('[data-module="tasks"]');
      if(navLink) navLink.click();
      else openEditTaskForm(task);
    }
  }
  ...
  function openAddMemberModal() {
    ...
    if(!available.length){toast('All users are already members','info'); return;}
    var userOpts=available.map(function(u){var uid=u.user_id||u.id; return '<option value="'+esc(uid)+'">'+esc(u.name||u.email)+'</option>';}).join('');
    var roleOpts=['Member','Lead','Reviewer','Observer'].map(function(r){return '<option>'+r+'</option>';}).join('');
    showModal(
      '<div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">'+
      ...
      '<div class="px-6 py-5 flex flex-col gap-4">'+
        '<div id="pm-status"></div>'+
        '<div><label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">User</label><select id="am-user" class="field text-sm">'+userOpts+'</select></div>'+
        '<div><label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Role</label><select id="am-role" class="field text-sm">'+roleOpts+'</select></div>'+
    ...
    document.getElementById('am-save').addEventListener('click', async function(){
      ...
      try {
        await apiMemberAdd(activeProject.id, uid, role);
        membersCache.push({project_id:activeProject.id, user_id:uid, role:role});
        closeModal();
        var lp=document.getElementById('proj-left-panel');
        if(lp){lp.innerHTML=renderLeftPanel(); wireLeftPanel();}
        toast('Member added ✓','success');
        apiActivityLog(activeProject.id,'added member',userName(uid));
      } catch(e){ modalStatus(e.message,false); btn.disabled=false; }
    });
  }
